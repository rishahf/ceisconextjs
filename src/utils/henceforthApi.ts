...
const Common = {
  ...
  checkDelivery: (id: string, lat: string, lng: string, country: string) =>
    requests.get(`Product/check/delivery?product_id=${id}&lat=${lat}&lng=${lng}${country ? `&country=${country}` : ''}&language=ENGLISH`),
  timer: () =>
    requests.get(`Homepage/user/deals_day/timer`),
  getMainKey: (id: string) =>
    requests.get(`User/main-keys/${id}?language=${Language}`)
}
...
const Products = {
  ...
  search: (search: any) =>
    requests.get(`User/search?search=${search}&language=${Language}`),
...
const Cart = {
  ...
  getCart: (pagination: number, limit: number) =>
    requests.get(`User/cart?limit=${limit ? limit : 6}&pagination=${pagination ? pagination : 0}&language=${Language}`),
  delete: (_id: any) =>
...
const Order = {
  ...
  priceDetails: () =>
    requests.get(`User/cart/price_details?language=${Language}`),
...
const Lang = {
  get: (id: string) =>
    requests.get(`User/main-keys/${id}?language=${Language}`),
  getKeyId: () =>
    requests.get(`User/main-keys?type=WEBSITE`)
}
const FILES = {
